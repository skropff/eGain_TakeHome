...
try:   
    file = input("Welcome to this troubleshooting chatbot! Each row in your file will be referred to by this chatabot based on the amount of rows preceding it. (e.g. The first row will be referred by this chatbot as row 0. Please enter the troubleshooting file name: ")
    file = file.strip()
    fd = open(file, mode='r')
...
def clean_up(current_state, traveled, issue_line):
    ...
    if current_state in traveled:
        sys.exit(f'A loop has been detected with state "{current_state}." The troubleshooting process could potentially last forever. ')
    ...
    for (first, second) in graph:
        if first == current_state:
            ...
            if tuple(second.split()) in double_array_words:
                sys.exit(f"Lines {graph[(first, second)][2]} and {double_array_words[tuple(second.split())]} have duplicate text and have the same first column values.")
            double_array_words[tuple(second.split())] = graph[(first, second)][2]
